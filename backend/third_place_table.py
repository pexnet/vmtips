...
ANNEX_C_COLUMNS: tuple[str, ...] = tuple(ANNEX_C_COLUMN_TO_MATCH.keys())

THIRD_PLACE_ASSIGNMENTS: dict[str, tuple[str, ...]] = {
...
def get_annex_c_match_mapping(groups: list[str] | tuple[str, ...] | set[str]) -> dict[int, str]:
    """Return {match_number: third_place_group} for eight advancing groups."""
    key = "".join(sorted({g.upper() for g in groups}))
    assignments = THIRD_PLACE_ASSIGNMENTS.get(key)
    if assignments is None:
        raise ValueError(f"No Annex C third-place assignment for groups: {key}")
    return {ANNEX_C_COLUMN_TO_MATCH[column]: group for column, group in zip(ANNEX_C_COLUMNS, assignments)}
